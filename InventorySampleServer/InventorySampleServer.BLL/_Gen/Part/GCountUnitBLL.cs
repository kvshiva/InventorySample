...
using InventorySampleServer.DAL.Part.CountUnit;

namespace InventorySampleServer.BLL._Gen.Part
{
	public class GCountUnitBLL<TEntity> : BaseBLL<TEntity> where TEntity : class
	{
		public GCountUnitBLL(string ConnectionString, GClaim Claim) : base(ConnectionString, Claim) { }
		...
		public override async Task<ResultDto> GetById(int Id)
		{
			...
			try
			{
				var dal = new CountUnitDAL<CountUnitListDto>(Connection, Transaction);
		...
		public override async Task<ResultDto> Add(TEntity Entity)
		{
			...
			var dal = new CountUnitDAL<CountUnitEntity>(Connection, Transaction);
			try
			{
				var Dto = Entity as CountUnitEntity ?? throw new Exception(MessageEnum.ثبت_رکورد_با_مشکل_مواجه_شد.EnumToString());
				var Validator = new CountUnitValidator();
				...
				var CountUnitId = await dal.Add(Dto);
				Transaction.Commit();

				return new Return().ReturnData(CountUnitId, StatusType.ثبت);
			}
			catch (Exception ex)
			{
				Transaction.Rollback();
