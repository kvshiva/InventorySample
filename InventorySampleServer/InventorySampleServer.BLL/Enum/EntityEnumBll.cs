...
using InventorySampleServer.Model.State.EntityEnum;

namespace InventorySampleServer.BLL.Enum
{
	public class EntityEnumBLL<TEntity> : BaseBLL<TEntity>  where TEntity : class
	{
		public EntityEnumBLL(string ConnectionString, GClaim Claim) : base(ConnectionString, Claim) { }
		...
		public override async Task<ResultDto> GetById(int Id)
		{
			...
			var dal = new EntityEnumDAL<EntityEnumEntity>(Connection, Transaction);
			try
			{
		...
		public override async Task<ResultDto> Add(TEntity Entity)
		{
			...
			try
			{
				var Dto = Entity as EntityEnumEntity ?? throw new Exception(MessageEnum.ثبت_رکورد_با_مشکل_مواجه_شد.EnumToString());
