...
using InventorySampleServer.Model.Store.InventoryVoucherSpecificationTypeEnum;

namespace InventorySampleServer.BLL.Enum
{
	public class InventoryVoucherSpecificationTypeEnumBLL<TEntity> : BaseBLL<TEntity>  where TEntity : class
	{
		public InventoryVoucherSpecificationTypeEnumBLL(string ConnectionString, GClaim Claim) : base(ConnectionString, Claim) { }
		...
		public override async Task<ResultDto> GetById(int Id)
		{
			...
			var dal = new InventoryVoucherSpecificationTypeEnumDAL<InventoryVoucherSpecificationTypeEnumEntity>(Connection, Transaction);
			try
			{
		...
		public override async Task<ResultDto> Add(TEntity Entity)
		{
			...
			try
			{
				var Dto = Entity as InventoryVoucherSpecificationTypeEnumEntity ?? throw new Exception(MessageEnum.ثبت_رکورد_با_مشکل_مواجه_شد.EnumToString());
