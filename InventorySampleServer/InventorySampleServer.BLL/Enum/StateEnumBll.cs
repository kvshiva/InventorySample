...
using InventorySampleServer.Model.State.StateEnum;

namespace InventorySampleServer.BLL.Enum
{
	public class StateEnumBLL<TEntity> : BaseBLL<TEntity>  where TEntity : class
	{
		public StateEnumBLL(string ConnectionString, GClaim Claim) : base(ConnectionString, Claim) { }
		...
		public override async Task<ResultDto> GetById(int Id)
		{
			...
			var dal = new StateEnumDAL<StateEnumEntity>(Connection, Transaction);
			try
			{
		...
		public override async Task<ResultDto> Add(TEntity Entity)
		{
			...
			try
			{
				var Dto = Entity as StateEnumEntity ?? throw new Exception(MessageEnum.ثبت_رکورد_با_مشکل_مواجه_شد.EnumToString());
