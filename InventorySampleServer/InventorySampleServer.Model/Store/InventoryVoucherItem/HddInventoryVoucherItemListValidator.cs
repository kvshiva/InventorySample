...
{
	public class HddInventoryVoucherItemListValidator : AbstractValidator<HddInventoryVoucherItemListDto>
	{
		public HddInventoryVoucherItemListValidator()
		{
			...
			RuleFor(e => e.InventoryVoucherId).NotEmpty().WithMessage(ValidationMessage.NotEmptyErrorMessage("شناسه سند"))
				.Must(e => e.IsDigit()).WithMessage(ValidationMessage.IsDigitErrorMessage());
			...
			/*RuleFor(e => e.InventoryVoucherItemSerialList).NotNull().Must(e => e?.Count > 0).WithMessage(ValidationMessage.NotEmptyListErrorMessage("سریال کالای سند انبار"))*/;
			RuleForEach(e => e.InventoryVoucherItemSerialList).SetValidator(new Model.Store.InventoryVoucherItemSerial.HddlInventoryVoucherItemSerialListValidator());
		}
	}
}
